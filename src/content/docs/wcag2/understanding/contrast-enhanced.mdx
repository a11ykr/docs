---
title: 1.4.6 명도 대비 (향상된)
description: WCAG 2.2 이해 성공 기준 1.4.6 명도 대비 (향상된)
head:
    - tag: title
      content: 성공 기준 이해 1.4.6 명도 대비 (향상된)
tableOfContents:
    maxHeadingLevel: 4
...
<Card title="성공 기준(SC)">
[텍스트](../../../glossary/text)와 [텍스트 이미지](../../../glossary/image-of-text)의 표현(presentation)을 위한 [명도대비율](../../../glossary/contrast-ratio)은 최소한 7:1 이상 되어야 한다. 단, 다음의 경우는 제외한다.
<dl>
    <dt>큰 텍스트</dt>
    <dd>
        [커다란](../../../glossary/large-scale) 텍스트와 텍스트 이미지가 최소한 4.5:1의 명도대비를 가지고 있다.
    </dd>
    <dt>부수적인</dt>
    <dd>
        비활성 [사용자 인터페이스 구성요소](../../../glossary/user-interface-component)의 일부, [순수한 장식](../../../glossary/pure-decoration), 사용자에게 보이지 않는, 또는 의미있는 다른 시각적 콘텐츠를 포함하고 있는 그림의 일부인 텍스트 또는 텍스트 이미지에는 어떠한 명도대비 요구사항도 없다.
    </dd>
    <dt>로고타입</dt>
    <dd>
...
이 성공 기준의 목적은 중간 정도의 저시력(고대비 보조 기술을 사용하지 않는 사람)자가 읽을 수 있도록 텍스트와 배경 사이에 충분한 대비를 제공하는 것이다. 색맹이 없는 사람의 경우 색상과 채도는 읽기 능력 평가에서 가독성에 거의 영향을 미치지 않거나 전혀 영향을 미치지 않는다(Knoblauch et al., 1991). 색각 이상은 명도 대비에 다소 영향을 미칠 수 있다. 따라서 권고안에서는 색상이 주요 요소가 아닌 방식으로 대비를 계산하여 색각 이상이 있는 사람들도 텍스트와 배경 사이에 적절한 대비가 있을 수 있도록 한다.
...
이 성공 기준은 텍스트에만 적용되지만 차트, 그래프, 다이어그램 및 기타 텍스트 기반이 아닌 정보에 표시되는 콘텐츠에 대해서도 유사한 문제가 발생하며 이는 [성공 기준 1.4.11 텍스트가 아닌 콘텐츠의 명도대비](non-text-contrast)에서 다룬다.
...
*   저시력인 사람들은 배경과 대비가 낮은 텍스트를 읽는 데 어려움을 겪는 경우가 많다. 색각 이상으로 대비가 더욱 낮아지는 경우 상황은 더욱 악화될 수 있다. 텍스트와 배경 사이에 최소 명도대비율을 제공하면 사람이 전체 색상 범위를 보지 못하더라도 텍스트를 더 쉽게 읽을 수 있다. 색깔이 전혀 보이지 않는 희귀한 개인에게도 효과가 있다.
