---
title: 3.2.2 입력 활성
description: WCAG 2.2 이해 성공 기준 3.2.2 입력 활성
head:
    - tag: title
      content: 성공 기준 이해 3.2.2 입력 활성
tableOfContents:
    maxHeadingLevel: 4
...
*   양식에는 미국 전화번호를 나타내는 필드가 포함되어 있다. 모든 전화번호에는 3자리 지역번호, 3자리 국번, 마지막 4자리 숫자가 있으며, 전화번호의 각 부분은 별도의 필드에 입력된다. 사용자가 한 필드 입력을 완료하면 초점이 자동으로 전화번호의 다음 필드로 이동한다. 전화 필드의 이러한 동작은 양식 시작 부분에서 사용자를 위해 설명된다.
