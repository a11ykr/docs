...
움직이거나 자동 업데이트되는 콘텐츠는 고정된 텍스트를 빠르게 읽는 데 어려움을 겪는 사람은 물론 움직이는 개체를 추적하는 데 어려움을 겪는 사람에게 장벽이 될 수 있다. 또한 스크린리더에 문제가 발생할 수도 있다.
