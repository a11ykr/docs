...
이 기준은 보조 기술 사용자의 요구를 직접 다루지 않는다. 이 기준의 목적은 일부 색상을 구별하지 못하는 시각 사용자가 콘텐츠를 이해할 수 있도록 보장하는 것이다. 보조기술 사용자에게 정보가 전달되는 방식은 [1.1.1 텍스트가 아닌 콘텐츠](non-text-content), [1.3.1 정보와 관계](info-and-relationships), [4.1.2 이름, 역할, 값](name-role-value) 등 다른 성공 기준에서 별도로 다룬다.
...
따라서 저작자는 방문한 링크에 대해 [1.3.1 정보와 관계](info-and-relationships)나 [4.1.2 이름, 역할, 값](name-role-value)을 달성할 수 없다.
