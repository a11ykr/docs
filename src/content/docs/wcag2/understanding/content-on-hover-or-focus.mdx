---
title: 1.4.13 마우스오버 혹은 키보드 초점을 받은 콘텐츠
...
<dl>
    <dt>해제 가능</dt>
    <dd>
        추가 콘텐츠가 [입력 오류](../../../glossary/input-error)를 전달하거나 다른 콘텐츠를 숨기거나 바꾸지 않는 한, 마우스 포인터로 가리키거나 키보드 초점을 이동하지 않고 추가 콘텐츠를 해제할 수 있는 [매커니즘](../../../glossary/mechanism)을 제공해야 한다.
    </dd>
    <dt>마우스오버</dt>
    <dd>
...
키보드 초점이나 마우스오버에 따라 추가 콘텐츠가 나타나고 사라지면 접근성 문제가 발생하는 경우가 많다. 이러한 문제의 원인은 다음과 같다.
...
또 다른 일반적인 상황은 플랫폼 설정이나 보조 기술을 통해 큰 포인터가 선택된 경우이다. 여기서 포인터는 추가 콘텐츠의 상당 부분을 가릴 수 있다. 두 상황 모두에서 콘텐츠를 완전히 보는 기술은 마우스 포인터를 호출자에서 새 콘텐츠로 직접 이동하는 것이다. 이 기능은 마우스 상호 작용에 대한 스크린리더 응답을 활용하는 사용자에게도 상당한 이점을 제공한다. 이 조건은 일반적으로 추가 콘텐츠가 대상과 겹치거나 인접하게 위치함을 의미한다.
...
*   모달 대화 상자는 키보드 초점을 받아야 하고 마우스오버 또는 초점을 받은 상태에서 나타나서는 안 되기 때문에 이 기준의 범위를 벗어난다. [성공 기준 3.2.1, 초점 활성](https://www.w3.org/TR/WCAG21/#on-focus)을 참조하라.
*   포인터 가리키기를 통해 실행될 수 있는 콘텐츠는 키보드 초점에 의해서도 실행될 수 있어야 한다. [성공 기준 2.1.1, 키보드](https://www.w3.org/TR/WCAG21/#keyboard)를 참조하라.
...
<figure class="understanding-figure" id="figure-mouse-tooltip-below">
            <img src="/docs/img/wcag22understanding/button-pointer-tooltip.png" alt="마우스 포인터가 위에 있고 버튼 아래에 툴팁이 표시된 버튼의 스크린샷" style="display:inline-block; margin-right:5px;"/>
            <img src="/docs/img/wcag22understanding/button-pointer.png" alt="마우스 포인터가 위에 있고 툴팁이 없는 버튼의 스크린샷"  style="display:inline-block;" />
            <figcaption>
...
<figure class="understanding-figure" id="figure-focused-button-no-tooltip">
    <img src="/docs/img/wcag22understanding/button-focus-indicator.png" alt="툴팁이 없고 초점 표시기가 있는 버튼의 스크린샷" />
    <figcaption>
...
<figure class="understanding-figure" id="figure-large-pointer-tooltip">
    <img src="/docs/img/wcag22understanding/button-pointer-on-tooltip.png" alt="큰 마우스 포인터가 호버한 버튼과 큰 포인터로 가려진 버튼 아래에 표시되는 툴팁의 스크린샷" style="display:inline-block; margin-right:5px;" />
    <img src="/docs/img/wcag22understanding/button-pointer-below-tooltip.png" alt="아래에 툴팁이 있는 버튼과 툴팁 하단에 큰 마우스 포인터가 있는 스크린샷" style="display:inline-block;" />
    <figcaption>
