...
CAPTCHA는 접근성 커뮤니티에서 논란의 여지가 있는 주제이다. [CAPTCHA의 비접근성](https://www.w3.org/TR/turingtest/) 논문에 설명된 대로 CAPTCHA는 자동화된 프로세스를 무력화하기 위해 본질적으로 인간 능력의 한계를 뛰어넘는다. 특정 장애가 있는 사용자는 모든 유형의 CAPTCHA를 해결할 수 없다. 그러나 CAPTCHA는 널리 사용되고 있으며 웹 콘텐츠 접근성 지침 실무 그룹은 CAPTCHA가 완전히 금지된다면 웹사이트는 CAPTCHA를 포기하는 대신 WCAG를 따르지 않을 것이라고 믿는다. 이는 훨씬 더 많은 장애가 있는 사용자에게 장벽을 만들 것이다. 이러한 이유로 실무 그룹은 대부분의 장애인 요구 사항을 충족하면서도 사이트에서 채택할 수 있는 방식으로 CAPTCHA에 대한 요구 사항을 구성하기로 결정했다. 특정 사이트에서 두 가지 다른 형태의 CAPTCHA를 요구하면 대부분의 장애인이 사용할 수 있는 양식을 찾을 수 있다.
...
<dl>
    <dt>데이터 차트</dt>
    <dd>막대 차트는 6월, 7월, 8월에 판매된 위젯의 수를 비교한다. 짧은 라벨은 "그림 1 - 6월, 7월, 8월 판매량"이라고 표시된다. 더 긴 설명은 차트의 유형을 식별하고, 차트에서 얻을 수 있는 것과 비교할 만한 데이터, 동향 및 함의에 대한 높은 수준의 요약을 제공한다. 가능하고 실용적인 경우, 실제 데이터가 표로 제공된다.</dd>
    <dt>연설의 녹음본</dt>
    <dd>오디오 클립 링크에는 "의장의 총회 연설"이라고 표시된다. 오디오 클립 링크 바로 다음에 텍스트 전사본 링크를 제공한다.</dd>
...
다음은 접근성 지침 실무 그룹에서 이 성공 기준 미준수로 간주하는 일반적인 실수이다.
